...
import { formatPrice, getProductType, getDisplayName } from '@/lib/utils'
import { useT } from '@/lib/i18n'

export default function CartDrawer({ open, onClose, cart, onUpdateQuantity, onRemoveItem, onCheckout }) {
  const t = useT()

  const subtotal      = cart.reduce((s, item) => s + item.price * item.quantity, 0)
  const deliveryFee   = subtotal >= 9000 ? 0 : 370
  const total         = subtotal + deliveryFee
  const totalQty      = cart.reduce((s, item) => s + item.quantity, 0)
  const freeDeliveryPct = Math.min((subtotal / 9000) * 100, 100)
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-[10px] text-[#3D4F3D]/50 tracking-wider uppercase mb-1">{getProductType(item.name) || item.category}</p>
                            <h4 className="text-[#3D4F3D] text-sm tracking-wide uppercase line-clamp-2">
                              {getDisplayName(item.name) || item.name}
                            </h4>
                        ...
                        <div className="flex items-center justify-between mt-4">
                          ...
                          <p className="text-[#3D4F3D] text-sm">{formatPrice(item.price * item.quantity)}</p>
                        </div>
                      </div>
                    </motion.div>
                  ))}
              ...
              {subtotal < 9000 && subtotal > 0 && (
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-[#3D4F3D]/50 tracking-wider">
                    {t('away_free', formatPrice(9000 - subtotal))}
                  </p>
                </div>
              )}
              {subtotal >= 9000 && (
