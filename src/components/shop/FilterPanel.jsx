...
export default function FilterPanel({ filters, onFiltersChange, allProducts, isOpen, onToggle, collections, onCollectionSelect, shopByCategories, onCategorySelect, noDesktopPanel }) {
  const t = useT()
  ...
  const filterContent = (
    <div className="space-y-6">
      ...
      {shopByCategories && onCategorySelect ? (
        /* New: broad category navigation (Macedonia) */
        <div className="space-y-3 lg:hidden border-t border-[#3D4F3D]/10 pt-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {shopByCategories.map(cat => (
              <button
                key={cat.value}
                onClick={() => { onCategorySelect(cat.value); onToggle(); }}
        ...
        /* Legacy: collection-based filtering */
        <div className="space-y-3 lg:hidden border-t border-[#3D4F3D]/10 pt-6">
          <Label className="text-[10px] text-[#3D4F3D]/50 tracking-[0.15em] uppercase">
            {t('shop_by')}
          </Label>
