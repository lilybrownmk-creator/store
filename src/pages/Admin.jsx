...
import { Package, ShoppingCart, Euro , Wallet, Banknote , AlertCircle, Plus, Pencil, Search, Clock, Truck, CheckCircle2, XCircle, Upload, LogOut } from 'lucide-react'
...
function ProductForm({ product, onSubmit, onClose }) {
  const [formData, setFormData] = useState(product || {
    ...
    category: 'Perfume', size: '', description: '', description_mk: '',
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <Textarea value={formData.description_mk} onChange={e => set({ description_mk: e.target.value })} className="mt-1" rows={2} />
        </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Revenue', value: `${Math.round(totalRevenue).toLocaleString('mk-MK')} ден`, icon: Banknote, bg: 'bg-green-100', ic: 'text-green-600' },
          ].map(stat => (
            <Card key={stat.label}>
