...
import { formatPrice, getProductType, getDisplayName, normaliseType } from '@/lib/utils'
...
const SUBCATEGORIES = {
  'Home Scent': ['Diffuser', 'Candle'],
  Body: ['Body Cream', 'Body Oil', 'Body Butter', 'Body Scrub', 'Shower Gel'],
  Hands: ['Hand Cream', 'Liquid Soap', 'Soap'],
  Hair: ['Shampoo', 'Hair Conditioner', 'Keratin Hair Mask'],
  'Self Care': ['Dead Sea Salt'],
}
...
// Ranked by actual Skopje sales data (Warehouse Skopje.csv — Sold column)
const bestSellers = [
  { rank:  1, category: 'Perfume',    name: 'Perfume Pink Pepper, Elemi, Cinnamon, Leather', size: '10 ml'  },
  { rank:  2, category: 'Perfume',    name: 'Perfume Black Vanilla',                          size: '10 ml'  },
  { rank:  3, category: 'Perfume',    name: 'Perfume Oud, Amber, Wood Blend',                 size: '10 ml'  },
  { rank:  4, category: 'Body',       name: 'Body Cream Black Pepper, Vetiver, Neroli, Amber',size: '195 ml' },
  { rank:  5, category: 'Hands',      name: 'Hand Cream Black Pepper, Vetiver, Neroli, Amber',size: '50 ml'  },
  { rank:  6, category: 'Perfume',    name: 'Perfume Tobacco, Vetiver, Amber',                size: '10 ml'  },
  { rank:  7, category: 'Perfume',    name: 'Perfume Black Vetiver, Amber',                   size: '10 ml'  },
  { rank:  8, category: 'Perfume',    name: 'Perfume Apple, Lotus',                           size: '10 ml'  },
  { rank:  9, category: 'Perfume',    name: 'Perfume White Blend',                            size: '10 ml'  },
  { rank: 10, category: 'Perfume',    name: 'Perfume Amber, Vanilla, Davana',                 size: '10 ml'  },
  { rank: 11, category: 'Body',       name: 'Liquid Soap Black Pepper, Vetiver, Neroli, Amber',size: '300 ml'},
  { rank: 12, category: 'Perfume',    name: 'Perfume Black Pepper, Vetiver, Neroli, Amber',   size: '50 ml'  },
  { rank: 13, category: 'Perfume',    name: 'Perfume Black Vanilla',                          size: '50 ml'  },
  { rank: 14, category: 'Perfume',    name: 'Perfume Pink Pepper, Elemi, Cinnamon, Leather',  size: '50 ml'  },
  { rank: 15, category: 'Perfume',    name: 'Perfume Orange and Jasmine',                     size: '50 ml'  },
  { rank: 16, category: 'Body',       name: 'Shower Gel Black Pepper, Vetiver, Neroli, Amber',size: '300 ml' },
  { rank: 17, category: 'Body',       name: 'Liquid Soap Black Vanilla',                      size: '300 ml' },
  { rank: 18, category: 'Body',       name: 'Body Cream Orange and Jasmine',                  size: '195 ml' },
  { rank: 19, category: 'Perfume',    name: 'Perfume Black Currant, Geranium, Amber, Leather',size: '10 ml'  },
  { rank: 20, category: 'Perfume',    name: 'Perfume Grapefruit, Basil, Petitgrain',          size: '10 ml'  },
  { rank: 21, category: 'Home Scent', name: 'Diffuser Black Vanilla',                         size: '85 ml'  },
  { rank: 22, category: 'Body',       name: 'Soap Black Vanilla',                             size: '75 g'   },
  { rank: 23, category: 'Perfume',    name: 'Perfume Rosemary and Lemon',                     size: '10 ml'  },
  { rank: 24, category: 'Perfume',    name: 'Perfume Vetiver, Lemon',                         size: '10 ml'  },
  { rank: 25, category: 'Perfume',    name: 'Perfume Rose, Jasmine, Narcissus, Musk',         size: '10 ml'  },
  { rank: 26, category: 'Perfume',    name: 'Perfume Red Grapefruit, Bergamot, Amber',        size: '10 ml'  },
  { rank: 27, category: 'Hands',      name: 'Hand Cream Pink Pepper, Elemi, Cinnamon, Leather',size: '50 ml' },
  { rank: 28, category: 'Perfume',    name: 'Perfume Tobacco, Cedarwood, Pepper',             size: '50 ml'  },
  { rank: 29, category: 'Perfume',    name: 'Perfume Leather, Sandalwood, Amber',             size: '50 ml'  },
  { rank: 30, category: 'Body',       name: 'Body Cream Pink Pepper, Elemi, Cinnamon, Leather',size: '195 ml'},
]

function isNewArrival(product) {
  const n = (product.name || '').toLowerCase()
  // New diffuser scents — Macedonia uses category 'Home Scent'
  if (product.category === 'Home Scent' || n.startsWith('diffuser')) {
    if (n.includes('amberwood') || (n.includes('vanilla') && n.includes('davana'))) return true
    if (n.includes('oud')) return true
    if (n.includes('blackcurrant') || n.includes('black currant')) return true
...
// Product type helpers (getProductType, getDisplayName, normaliseType) imported from @/lib/utils

// Match a product against a category filter value (handles both broad DB cats and specific types)
function matchesCategoryFilter(product, cat) {
  const c = product.category || ''
  const type = normaliseType(getProductType(product.name || ''))
  switch (cat) {
    case 'all':        return true
    case 'bestsellers':return isBestSeller(product)
    // Broad DB categories
    case 'Perfume':    return c === 'Perfume'
    case 'Home Scent': return c === 'Home Scent'
    case 'Body':       return c === 'Body'
    case 'Hands':      return c === 'Hands'
    case 'Hair':       return c === 'Hair'
    case 'Self Care':  return c === 'Self Care'
    // Specific product types (name-prefix based)
    default:           return type === cat
  }
}
    ...
    >
      ...
      <div className="text-center space-y-0.5 px-2">
        <p className="text-[9px] text-[#3D4F3D]/50 tracking-[0.15em] uppercase">
          {getProductType(product.name) || product.category}{product.size && ` - ${product.size}`}
        </p>
        <h3 className="text-xs text-[#3D4F3D] tracking-wide leading-snug min-h-[32px] line-clamp-2">
          {getDisplayName(product.name) || product.name}
        </h3>
        <p className="text-sm text-[#3D4F3D] font-light pt-1">{formatPrice(product.price)}</p>
      </div>
      <button
...
function AllProductsSections({ products, onAddToCart, onToggleWishlist, isInWishlist, onProductClick, onCategorySelect }) {
  const t       = useT()
  ...
  return (
    <div className="space-y-10 pt-4">
      ...
      {/* Category rows */}
      {sectionCats.map(cat => {
        const catProducts = products.filter(p => matchesCategoryFilter(p, cat.value))
...
export default function Shop() {
  ...
  const priceRangeProducts = products.filter(product => {
    const matchesCat        = matchesCategoryFilter(product, filters.category)
  ...
  const filteredProducts = products
    .filter(product => {
      ...
      const matchesCategory = matchesCategoryFilter(product, filters.category)
  ...
  const handleCategorySelect = value => {
    setFilters(prev => ({ ...prev, category: value, collections: [], priceRange: [0, maxPrice], bestsellersOnly: false, newArrivalsOnly: false, inStockOnly: false }))
    ...
    <div className="min-h-screen bg-[#F5F3F0]">
          ...
          <div className="mx-1 flex items-end pb-3 justify-between h-52 md:items-center md:pb-0 md:h-56">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-1 md:top-[28%] md:-translate-y-1/2 w-[72%] sm:w-auto">
              <div className="text-center">
                ...
                />
                <a
                  href="https://share.google/sFY2iroYnKXttXUwZ"
                  target="_blank"
      ...
      <div className="sticky top-0 z-40 bg-[#F5F3F0] border-b border-[#3D4F3D]/10 shadow-sm">
        <div className="px-4 lg:px-8">
          {/* Mobile: single row — All Products · Best Sellers | broad categories */}
      ...
      <div className="mx-2 pt-2 pr-3 pb-3 pl-3 opacity-45 lg:px-8 flex items-center justify-between">
        ...
        <p className="text-[#3D4F3D]/60 text-xs tracking-wider hidden sm:block">
          {t('free_delivery_banner', '9.000 ден')}
        </p>
      </div>
          ...
          <div className="grid lg:grid-cols-[280px_1fr] gap-8">
            <div className="hidden lg:block space-y-4 lg:sticky lg:top-6 lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto scrollbar-hide">
              <FilterPanel
                ...
                shopByCategories={BROAD_CATS}
                onCategorySelect={handleCategorySelect}
              />
              {/* ── Shop By broad categories — below filter panel ── */}
            ...
            <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2">
              {recentlyViewed.map(p => (
                ...
                >
                  ...
                  <p className="text-[9px] text-[#3D4F3D] font-medium">{formatPrice(p.price)}</p>
                </button>
              ))}
            </div>
          </div>
      ...
      <footer className="bg-[#2F4E3C] text-white px-4 pt-4 pb-36 md:py-5 lg:px-8">
        ...
        <div className="hidden md:grid max-w-6xl mx-auto grid-cols-3 gap-4 items-center">
          <div>
            ...
            <a href="mailto:Skopje@springsioux.com" className="block text-[10px] font-light text-white/70 hover:text-white transition-colors">Skopje@springsioux.com</a>
            <a href="tel:+38971310123" className="block text-[10px] font-light text-white/70 hover:text-white transition-colors mt-0.5">+389 71 310 123</a>
          </div>
          <div className="text-center">
          ...
          <div className="text-right">
            <p className="text-[9px] tracking-[0.1em] mb-1.5 text-white/50 uppercase">{t('free_delivery_banner', '9.000 ден')}</p>
            <p className="text-[9px] tracking-[0.1em] text-white/50 uppercase">HORECA &amp; HOSPITALITY</p>
            <a href="mailto:Skopje@springsioux.com" className="block text-[10px] font-light text-white/70 hover:text-white transition-colors mt-0.5">Skopje@springsioux.com</a>
          </div>
        </div>
              ...
              >
                <div className="pt-5 text-white/70 text-[10px] leading-relaxed max-w-2xl mx-auto space-y-3">
                  ...
                  {openLegal === 'impressum' && (<>
                    ...
                    <p><strong className="text-white/60">Database:</strong> Supabase Inc.</p>
                  </>)}
                  ...
                  {openLegal === 'terms' && (<>
                    ...
                    <p><strong className="text-white/60">5. Warranty</strong><br />Statutory warranty rights apply. Defective goods may be reported within 2 years of purchase.</p>
                    <p><strong className="text-white/60">6. Complaints</strong><br />Submit complaints to Skopje@springsioux.com. Written complaints are answered within 30 days and kept for 5 years.</p>
                  </>)}

                  {openLegal === 'privacy' && (<>
                    <p className="text-[9px] tracking-[0.2em] text-white/40 uppercase mb-2">Privacy Policy</p>
                    <p><strong className="text-white/60">Controller:</strong> SANE STUDIO KFT · Skopje@springsioux.com</p>
                    <p><strong className="text-white/60">Data collected & purposes:</strong><br />
                      — Order fulfilment: name, e-mail, phone, delivery address (legal basis: contract performance; retention: 5 years)<br />
                      — Invoicing: invoice data (legal basis: legal obligation; retention: 8 years)<br />
                      — Newsletter/marketing: e-mail (legal basis: consent, withdrawable at any time)
                    </p>
                    <p><strong className="text-white/60">Processors:</strong> Vercel Inc. (hosting) · Supabase Inc. (database) · Revolut Ltd. (payments) · Resend Inc. (e-mail)</p>
                    <p><strong className="text-white/60">Your rights:</strong> access · rectification · erasure · portability · objection · consent withdrawal · restriction</p>
                    <p><strong className="text-white/60">Cookies:</strong> This site uses session cookies (cart, settings) active until the browser is closed. Statistical cookies require consent.</p>
                  </>)}
                ...
                <div className="flex-1 p-8 lg:p-12 overflow-y-auto">
                  <div className="mb-8">
                    <div className="border-t-2 border-b-2 border-dashed border-[#3D4F3D] py-4 mb-4">
                      <DialogTitle className="text-3xl lg:text-4xl text-[#3D4F3D] font-bold tracking-wider leading-tight uppercase">
                        {getDisplayName(selectedProduct.name) || selectedProduct.name}
                      </DialogTitle>
                    </div>
                    <div className="border-t-2 border-b-2 border-dashed border-[#3D4F3D] py-3 flex items-center justify-between">
                      <p className="text-sm text-[#3D4F3D] font-bold tracking-wider uppercase">
                        {getProductType(selectedProduct.name) || selectedProduct.category}{selectedProduct.size && ` ${selectedProduct.size}`}
                      </p>
                      <p className="text-xl text-[#3D4F3D] font-bold">{formatPrice(selectedProduct.price)}</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-8 mt-12">
                    ...
                    <p className="text-sm text-[#3D4F3D] leading-relaxed tracking-wide">
                      {descriptionLang === 'MK'
                        ? (selectedProduct.description_mk || selectedProduct.description || 'Зиелински & Розен парфемите се унисекс, дизајнирани да се развиваат и да го открийат своето уникатно суштество на вашата кожа.')
                          ...
                          <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                            {similar.map(p => (
                              ...
                              >
                                ...
                                <p className="text-[8px] text-[#3D4F3D] font-medium">{formatPrice(p.price)}</p>
                              </button>
                            ))}
                          </div>
                        </div>
