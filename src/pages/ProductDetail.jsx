    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="px-5 pt-5 pb-4">
        ...
        <div className="border-b-2 border-dashed border-[#3D4F3D] pb-3 mb-4 flex items-center justify-between">
          ...
          <p className="text-xl text-[#3D4F3D] font-light">{formatPrice(product.price)}</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-sm text-[#3D4F3D] leading-relaxed tracking-wide">
            {descLang === 'MK' ? (product.description_mk || product.description) : product.description}
          </p>
        </div>
      </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {similar.map(p => (
              <button key={p.id} onClick={() => navigate(`/product/${p.id}`, { replace: true })}
                ...
                <p className="text-[9px] text-[#3D4F3D] leading-tight line-clamp-2">{getDisplayName(p.name) || p.name}</p>
                <p className="text-[10px] text-[#3D4F3D] font-medium mt-0.5">{formatPrice(p.price)}</p>
              </button>
            ))}
          </div>
        </div>
